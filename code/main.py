#!/usr/bin/env python3
"""
main.py: “先大类，后小类” 分层训练流水线 (修复 Positional Encoding 长度不足问题)
"""
...
# [修复] 定义最大时间步长，需 >= preprocess_dataset.py 中的 max_len (默认60)
...
def main():
    setup_logging()
    print("="*60)
    print("🚀 启动分层训练流水线 (Fix: Temporal Steps Size)")
    print("="*60)
    ...
    print(f"   最大时间步长: {MAX_TEMPORAL_STEPS}")
    ...
    major_model = DualStreamSpatio_TemporalFusionNetwork(
        ...
        temporal_steps=MAX_TEMPORAL_STEPS,  # <--- Fix
    ...
    major_trainer = Trainer(
        ...
        model_init_params={
            ...
            'in_channels_static': sta_ch,
            'num_classes': len(major_map),
            'temporal_steps': MAX_TEMPORAL_STEPS,  # <--- Fix: 必须传给 trainer 用于重建模型
            'dropout': config.get('model.dropout', 0.25),
            'classifier_hidden_dims': config.get('model.classifier.hidden_dims', [128, 64, 32])
        }
    )
    ...
    print(f"✅ 大类模型 K-Fold 训练完成 | 平均精度: {kfold_results['mean_metrics'].get('accuracy', 0):.4f}")
    ...
    for major_name, major_id in major_map.items():
        ...
        if len(train_indices) >= major_kfold_n_splits:
            ...
            sub_trainer = Trainer(
                model=sub_model,
                train_dataloader=DataLoader(train_subset, shuffle=True, batch_size=detail_cfg['batch_size'], collate_fn=collate_fn, drop_last=use_drop_last, **common_cfg),
            ...
            sub_trainer.train(num_epochs=detail_cfg['epochs'], learning_rate=detail_cfg['learning_rate'], weight_decay=detail_cfg['weight_decay'], patience=detail_cfg['patience'])
        
        with open(sub_model_dir / 'class_mapping.json', 'w', encoding='utf-8') as f:
            json.dump({'major_class': major_name, 'major_id': major_id, 'local_to_global_map': local_to_global}, f, ensure_ascii=False)
